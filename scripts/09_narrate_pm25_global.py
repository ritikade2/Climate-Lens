...
def main() -> None:
    ...
    user_query = "Summarize the latest detected GLOBAL PM2.5 drift and top contributors (trust-aware)."
    ...
    if g.empty:
        ...
        _audit(s, request_id, user_query, allowed=False, refusal=refusal, response_text=response_text, event_id=event_id, cp=cp)
        print(response_text)
        return
...
def render_narrative(
    ...
    ep = "NA" if (pd.isna(effect_pct)) else f"{float(effect_pct):3f}"
    cp_s = str(pd.to_datetime(cp).date())
    txt = f"""GLOBAL PM2.5 drift summary (trust-aware)
            ...
            - Attribution is computed from pre/post mean deltas per country multiplied by population (not emissions, not policy, not source apportionment).
