...
# Config / constants
# -----------------------------
# 1:1 match to PM25 global script, but for precipitation global-popw KPI
KPI_ID = "precip_country_monthly_sum_global_popw"
SOURCE_ID = "openmeteo"
GEO_ID = "GLOBAL"
...
def main() -> None:
    ...
    if df.empty:
        raise RuntimeError(
            "No GLOBAL precip rows found. Run the script that builds " 
            "precip_country_monthly_sum_global_popw first."
        )
    ...
    # coverage score (population coverage)
    ...
    # volatility on values (seasonality agnostic; spikes)
    dv = g2["value"].diff()
    ...
    # z_abs <= 1.5 -> ~1.0 score, z_abs >= 4 -> ~0.0
    g2["volatility_score"] = (
    ...
    # NOTE: PM25 script writes 'missingess_score' (typo). Keep it identical for 1:1 alignment.
    out = pd.DataFrame(
        {
            "kpi_id": KPI_ID,
            "geo_id": GEO_ID,
    ...
    updates["data_quality_flag"] = np.where(
        (updates["confidence_grade"].isin(["A", "B"]))
        & (updates["coverage_pct"] >= 0.90)
        & (g2["volatility_score"].values >= 0.85),
        "pass",
        "warn",
    )
    ...
    try:
        con.register(
            "global_updates",
            updates[["kpi_id", "geo_id", "date", "source_id", "coverage_pct", "data_quality_flag"]],
        )
        con.execute(
            """
    ...
    print(f"GLOBAL integrity scored for {len(out):,} month.")
