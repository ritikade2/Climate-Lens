...
def main() -> None:
    ...
    if g.empty:
        raise RuntimeError("GLOBAL integrity missing at cp_start. Run scripts/07b_score_integrity_pm25_global.py first.")
    ...
    print(f"Base pct sum (top_n only) = {out_base['contribution_pct'].sum():.3f}") # means top 25 countries cover the base contribution mass; remaining in the long tail
    print(f"Conf pct sum (top_n only) = {out_conf['contribution_pct'].sum():.3f}") # due to strict filtering, lots of countries were dropped. So remaining pool is smaller and the top 25 can easily become "all of them".
